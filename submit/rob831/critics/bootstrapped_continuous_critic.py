...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            # For each grad step in the update
            if i % self.num_grad_steps_per_target_update == 0:
                # Recompute the target values by querying the critic network
                v_s_prime = self.forward_np(next_ob_no)
                targets   = reward_n + self.gamma * v_s_prime * (terminal_n.astype("bool") == False).astype("int") # Look for terminals, if we are at the end, there is no last state then it is just the reward

            # Compute the empirical value of the critic network (based on the current parameters, what the value of the state is)
            v_s = self.forward(ptu.from_numpy(ob_no))

            # Update the parameters with gradient descent
            self.optimizer.zero_grad()
            # Compute the loss
            loss = self.loss(ptu.from_numpy(targets), v_s)
            loss.backward()
            self.optimizer.step()
