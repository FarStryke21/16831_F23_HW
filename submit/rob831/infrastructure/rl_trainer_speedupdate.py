...
def collect_trajectories_worker(env, policy, num_transitions, ep_len, result_queue):
    paths, envsteps_this_batch = utils.sample_trajectories(env, policy, num_transitions, ep_len)
    result_queue.put((paths, envsteps_this_batch))
...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        for _ in range(num_threads):
            # Create a new process, each running the worker function
            p = multiprocessing.Process(target=collect_trajectories_worker, args=(self.env, collect_policy, num_transitions_to_sample, self.params['ep_len'], result_queue))
            processes.append(p)
            p.start()
        ...
        for _ in range(num_threads):
            ...
            paths.extend(result[0])
            envsteps_this_batch += result[1]
    ...
    def perform_logging(self, itr, paths, eval_policy, train_video_paths, all_logs):
        ...
        if self.log_metrics:
            ...
            print(f"No. of Train Returns : {len(train_returns)} | Train Returns : {train_returns}")
            print(f"No. of Eval Returns : {len(eval_returns)} | Eval Returns : {eval_returns}")
            ...
            logs["Performance"] = logs["Eval_AverageReturn"]/logs["Initial_DataCollection_AverageReturn"]
